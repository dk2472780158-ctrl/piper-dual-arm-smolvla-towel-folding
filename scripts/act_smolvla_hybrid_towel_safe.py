#!/usr/bin/env python3
"""Dual-Piper towel folding: ACT primary control + guarded SmolVLA takeover.
...
close to ACT.  Otherwise the exact ACT action is sent for that control tick.
"""
...
def validate_execution(
    ...
    *,
    ...
    if grip_step > args.gripper_step_limit:
        raise RuntimeError(
    ...
    if grip_tracking > args.gripper_tracking_limit:
        raise RuntimeError(
...
def smol_is_acceptable(
    ...
    step_joint, step_grip = max_parts(np.abs(candidate - previous))
    ...
    if step_grip > args.gripper_step_limit:
        return False, f"filtered_gripper_step={step_grip:.6f}", disagree_joint, disagree_grip, raw_step_joint
    ...
    if disagree_grip > args.gripper_disagreement:
        return False, f"filtered_gripper_disagreement={disagree_grip:.6f}", disagree_joint, disagree_grip, raw_step_joint
        ...
        for step in range(args.max_actions):
            ...
            if smol_active:
                ...
                if candidate_tensor is None:
                    ...
                    ok, reason, dj, dg, raw_step = smol_is_acceptable(
                        raw_candidate,
                        candidate,
                        act_action,
                        previous,
                    ...
                    if ok:
                        live_tracking, live_grip_tracking = max_parts(
                        ...
                        elif live_grip_tracking > args.gripper_tracking_limit:
                            ok = False
                            reason = (
                                "gripper_tracking_gate="
                                f"{live_grip_tracking:.6f}"
                            )
                    if ok:
                        target = candidate
                        source = "SMOL"
                        ...
                        consecutive_rejects = 0
                        detail = (
                            f"lowpass_alpha={args.smol_lowpass_alpha:.2f} "
            ...
            print(
                ...
                f"global_alpha={args.global_lowpass_alpha:.2f} {detail}",
                flush=True,
            )

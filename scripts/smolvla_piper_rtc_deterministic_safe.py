...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reset the RNG immediately inside every predict_action_chunk call "
            "so async thread scheduling cannot change the sampled flow noise."
        ),
    )
...
def main() -> int:
    ...
    if args.deterministic_inference:
        ...
        def deterministic_predict_action_chunk(batch, noise=None, **kwargs):
            if noise is None:
                torch.manual_seed(args.seed)
                if torch.cuda.is_available():
                    torch.cuda.manual_seed_all(args.seed)
            return original_predict_action_chunk(batch, noise=noise, **kwargs)

        policy.predict_action_chunk = deterministic_predict_action_chunk
        print(
            f"Deterministic inference enabled inside predict_action_chunk; seed={args.seed}",
            flush=True,
        )

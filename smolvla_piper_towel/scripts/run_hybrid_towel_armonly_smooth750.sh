...
HYBRID_SCRIPT="$SMOL_ROOT/scripts/act_smolvla_hybrid_towel_armonly_smooth.py"
...
RUN_DIR="$EXPERIMENT_ROOT/armonly_smooth_one_click_runs/$RUN_ID"
...
echo "300步后：SmolVLA低通条件接管，不合格动作回退ACT"
    echo "关节协作版：ACT始终控制夹爪；Smol仅参与12个手臂关节"
echo "============================================================"
...
if pgrep -af '[l]erobot-record|[r]obot_client|[a]ct_smolvla_hybrid_towel_(safe|smooth|phase_smooth|armonly_smooth).py' >/dev/null; then
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/armonly_smooth_one_click_runs/latest"

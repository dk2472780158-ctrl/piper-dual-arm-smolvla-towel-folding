...
HYBRID_SCRIPT="$SMOL_ROOT/scripts/act_smolvla_hybrid_towel_smooth.py"
...
RUN_DIR="$EXPERIMENT_ROOT/balanced_one_click_runs/$RUN_ID"
...
echo "300步后：SmolVLA低通条件接管，不合格动作回退ACT"
    echo "折中平滑版：全局低通alpha=0.60，放宽速度/加速度以保持抓取轨迹"
echo "============================================================"
...
if pgrep -af '[l]erobot-record|[r]obot_client|[a]ct_smolvla_hybrid_towel_(safe|smooth).py' >/dev/null; then
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/balanced_one_click_runs/latest"

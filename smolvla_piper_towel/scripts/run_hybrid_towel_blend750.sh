...
source /workspace/pyshim/env.sh
RESET_SCRIPT="$ACT_ROOT/reset_piper_pose.py"
HYBRID_SCRIPT=/workspace/pyshim/hy.py
...
RUN_DIR="$EXPERIMENT_ROOT/blend_one_click_runs/$RUN_ID"
...
echo "============================================================"
echo "双Piper毛巾折叠：SmolVLA 一键完整750步"
echo "运行编号：$RUN_ID"
echo "前300步：安全栈保底主控"
echo "300步后：SmolVLA连续权威混合（无二进制切换）"
echo "  authority∈[0,0.9]；不合格/越界动作使authority衰减，命令平滑回到安全栈"
echo "============================================================"
...
[[ -s "$ACT_MODEL/model.safetensors" ]] || fail "基线V4模型不完整：$ACT_MODEL"
...
# go2rtc RGB 旁路：piper_dual.get_observation → fanout TCP（见 mine-world-arm USER_CAMERA_GO2RTC.md）
# PROJECT_ROOT 指 SMOL 树，避免 fanout env 把 ACT src 插到 PYTHONPATH 前面盖住 SmolVLA。
PROJECT_ROOT="${SMOL_ROOT}"
# shellcheck source=/home/databall_02/VLA/piper_fanout/scripts/piper_lerobot_fanout_env.sh
source "${PIPER_FANOUT_ROOT:-/home/databall_02/VLA/piper_fanout}/scripts/piper_lerobot_fanout_env.sh"
echo "fanout ENABLE_PIPER_RGB_PUBLISH=${ENABLE_PIPER_RGB_PUBLISH}"



LEFT_DEV="$(readlink -f /dev/video18 2>/dev/null || true)"
MIDDLE_DEV="$(readlink -f /dev/video12 2>/dev/null || true)"
RIGHT_DEV="$(readlink -f /dev/video4 2>/dev/null || true)"
...
python -B "$HYBRID_SCRIPT" \
    ...
    --smol-correction-limit 0.15 \
    --correction-alpha 0.5 \
    --correction-step-limit 0.02 \
...
SAFE_COUNT="$(grep -c 'source=SAFE ' "$RUN_LOG" 2>/dev/null || true)"
SMOL_COUNT="$(grep -c 'source=SmolVLA ' "$RUN_LOG" 2>/dev/null || true)"
...

{
    ...
    echo "safe_guard_ticks=$SAFE_COUNT"
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/blend_one_click_runs/latest"

echo
echo "===== 本次结果 ====="
echo "安全栈保底动作：$SAFE_COUNT"

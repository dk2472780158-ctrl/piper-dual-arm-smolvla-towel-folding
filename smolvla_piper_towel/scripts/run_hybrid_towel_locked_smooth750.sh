...
HYBRID_SCRIPT="$SMOL_ROOT/scripts/act_smolvla_hybrid_towel_locked_smooth.py"
...
RUN_DIR="$EXPERIMENT_ROOT/locked_smooth_one_click_runs/$RUN_ID"
...
echo "300步后：SmolVLA低通条件接管，不合格动作回退ACT"
    echo "锁定平滑版：ACT控制夹爪；控制器连续判定后才成段切换"
echo "============================================================"
...
if pgrep -af '[l]erobot-record|[r]obot_client|[a]ct_smolvla_hybrid_towel_(safe|smooth|phase_smooth|armonly_smooth|locked_smooth).py' >/dev/null; then
...
python -B "$HYBRID_SCRIPT" \
    ...
    --global-lowpass-alpha 0.60 \
    --smooth-joint-step-limit 0.070 \
    --smooth-joint-accel-limit 0.012 \
    --smooth-gripper-step-limit 0.006 \
    --smooth-gripper-accel-limit 0.001 \
    --smooth-start-step 300 \
    --smol-accept-streak 3 \
    --smol-reject-streak 3 \
    --source-min-dwell 8 \
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/locked_smooth_one_click_runs/latest"

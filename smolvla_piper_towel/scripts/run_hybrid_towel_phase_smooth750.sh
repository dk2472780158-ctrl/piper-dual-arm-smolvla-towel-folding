...
HYBRID_SCRIPT="$SMOL_ROOT/scripts/act_smolvla_hybrid_towel_phase_smooth.py"
...
RUN_DIR="$EXPERIMENT_ROOT/phase_smooth_one_click_runs/$RUN_ID"
...
echo "300步后：SmolVLA低通条件接管，不合格动作回退ACT"
    echo "分阶段平滑版：前300步保持原版ACT；Smol接管后才启用平滑"
echo "============================================================"
...
if pgrep -af '[l]erobot-record|[r]obot_client|[a]ct_smolvla_hybrid_towel_(safe|smooth|phase_smooth).py' >/dev/null; then
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/phase_smooth_one_click_runs/latest"

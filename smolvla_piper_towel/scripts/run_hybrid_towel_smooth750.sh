...
RUN_DIR="$EXPERIMENT_ROOT/smooth_one_click_runs/$RUN_ID"
...
echo "300步后：SmolVLA低通条件接管，不合格动作回退ACT"
    echo "平滑实验版：全局低通alpha=0.35，关节速度/加速度双限制"
echo "============================================================"
...
python -B "$HYBRID_SCRIPT" \
    ...
    --global-lowpass-alpha 0.35 \
    --smooth-joint-step-limit 0.040 \
    --smooth-joint-accel-limit 0.004 \
    --smooth-gripper-step-limit 0.003 \
    --smooth-gripper-accel-limit 0.0004 \
...
ln -sfn "$RUN_DIR" "$EXPERIMENT_ROOT/smooth_one_click_runs/latest"
